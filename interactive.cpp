...
static propertyconfig* getPropertyRequested(xmlDocPtr doc, xmlNodePtr properties, char* prop){
...
static zonevalue* convZoneValue(xmlDocPtr doc, xmlNodePtr emulator){
...
static xmlChar* getZoneValue(zonevalue* zoneTovalue, xmlChar* zonetype, xmlChar* areaname){
...
static int requestRow(propertyconfig* propertyStruct, zonevalue* zoneTovalue){
...
static xmlChar* treatEnumValue(propertyconfig* propertyStruct, char* val){
...
static int32_t treatIntValue(propertyconfig* propertyStruct, char* val){
...
static float treatFloatValue(propertyconfig* propertyStruct, char* val){
...
static int getProperty(emulator::EmulatorMessage* message, char* prop, xmlNodePtr properties, xmlNodePtr cur, xmlDocPtr doc){
...
static int setProperty(emulator::EmulatorMessage* message, char* prop, char* val, xmlNodePtr properties, xmlNodePtr cur, xmlDocPtr doc){
...
static int getOrset(emulator::EmulatorMessage* message, xmlNodePtr properties, xmlNodePtr cur, xmlDocPtr doc){
	...
	if(!strcmp((const char*)&type, "get")){
		if(getProperty(message, (char*)&pro, properties, cur, doc) == -1){
			xmlFreeDoc(doc);
			return -1;
		}
	}
		...
		printf("value? ");
		scanf("%s", val);
		if(setProperty(message, (char*)&pro, (char*)&val, properties, cur, doc) == -1){
	 		xmlFreeDoc(doc);
			return -1;
		}
	}
...
static void afficher_resultat(emulator::EmulatorMessage* message){
...
int start_interactive(xmlDocPtr doc, xmlNodePtr cur, char* ip, xmlNodePtr properties){

	int							sock;
	emulator::EmulatorMessage 	message;
	emulator::EmulatorMessage	message_recv;

	if(getOrset(&message, properties, cur, doc) == -1){
		return -1;
	}
